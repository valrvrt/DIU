...
class GameLoop:
    ...
    def _human_turn(self, player_id: str):
        """Handle human player input (simplified for now)."""
        print(f"\n👤 Your turn! (Type 'help' for options, 'skip' to let bot play)")

        # For now, let the bot play for the human too
        # This can be expanded later for full human interaction
        input_str = input("Action: ").strip().lower()

        if input_str == "skip" or input_str == "":
            print("Bot playing for you...")
            ...
            self._human_turn(player_id)  # Ask again
        else:
            print("Invalid command. Letting bot play...")
            result = self.bot.take_turn(player_id, self.game)
